...
// 대시보드 자동 갱신 (2026-09-07) — 띄워놓고 보는 화면(벽 모니터·내 대시보드)이 낡지 않게.
//   ★ 1시간인 이유: 여기 걸리는 화면들(계약 진행현황·완제품 재고·생산 실적)은 출하·검사 단위로
//     움직여 분 단위 변화가 없다. 실시간성이 필요한 화면은 이미 자기 주기를 갖고 있다
//     (실시간 재고 5초 · 포장 현황 10초 · 온습도 15초) — 그쪽은 이 상수를 쓰지 않는다.
//   ★ 폴링은 반드시 silent 로 — 로딩 스피너를 다시 띄우면 보고 있던 화면이 1시간마다 깜빡인다.
export const DASHBOARD_POLL_MS = 60 * 60 * 1000
...
// 비활성 칸 = 버튼 없이 '-' 고정 (Test1Section) + 저장값도 '-' 강제 (InspectionForm 초기화).
//   Ring(dim_a) 고정 (2026-09-12, 사용자 요청) — Ring 검사는 하지 않는데 OK 가 눌려 기록되던 것을 막는다.
export const DIM_DISABLED = [true, false, false, false, false]
