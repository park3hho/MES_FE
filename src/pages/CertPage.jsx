...
function Logo({ size = 130 }) {
  return <img src="/FaradayDynamicsLogo.png" width={size} alt="Faraday Dynamics" style={{ display: "block" }} />
}
            ...
            <div key={nIdx} className={s.nodeRow}>
              ...
              <div className={s.nodeContent} style={{ paddingBottom: isNodeLast ? 4 : 10 }}>
                <div className={s.nodeHead}>
                  <span className={s.nodeCode} style={{ color: isFirst ? ORANGE : "#b0b0b0" }}>{node.process}</span>
      ...
      {items.map((item, idx) => {
        ...
        return (
          <div key={item.col}>
            <div className={s.tlRow}>
              <div className={s.tlLeft}>
                ...
                {!last && !(item.isBo && boBranches?.length > 0) && (
                  <div style={{
                    width: first ? 1.5 : 1, flex: 1, minHeight: 20,
                    background: first ? `linear-gradient(to bottom, ${ORANGE}, ${BORDER})` : BORDER,
                  }} />
                )}
              </div>
              <div className={s.tlContent} style={{ paddingBottom: (item.isBo && boBranches?.length > 0) ? 4 : last ? 8 : 14 }}>
                <div className={s.tlProc}>
                  <span className={s.tlCode} style={{ color: first ? ORANGE : "#b0b0b0" }}>{item.proc}</span>
                ...
                {item.isBo && boBranches?.length > 0 && (
                  <div className={s.tlToggle} onClick={() => setBoOpen(prev => !prev)}>
                    <span className={s.tlArrow} style={{ transform: boOpen ? "rotate(90deg)" : "rotate(0)" }}>▶</span>
            ...
            {item.isBo && boBranches?.length > 0 && (
              <div className={s.branchWrap} style={{ maxHeight: boOpen ? 3000 : 0, opacity: boOpen ? 1 : 0 }}>
      ...
      <div className={s.prodHeader} onClick={onToggle}>
        <div className={s.prodLeft}>
          ...
          <div>
            ...
            <div className={s.prodSub}>Unit {idx + 1} of {total}</div>
          </div>
        </div>
    ...
    <div className={s.page}>
      <div className={s.certWrap}>
        ...
        <div className={s.infoGrid}>
          <div><div className={s.infoLabel}>Shipment</div><div className={s.infoValue}>{data?.ob_lot_no}</div></div>
          <div><div className={s.infoLabel}>Date</div><div className={s.infoValue}>{formatDate(data?.created_at)}</div></div>
          <div><div className={s.infoLabel}>Boxes</div><div className={s.infoValue}>{data?.boxes?.length || 0}</div></div>
          <div><div className={s.infoLabel}>Total units</div><div className={s.infoValue}>{totalProducts}</div></div>
        </div>
          ...
          return (
            <div key={bIdx}>
              <div className={s.divRow}>
                ...
                <span className={s.divLabel}>{box.bx_lot_no} — {count} unit{count > 1 ? "s" : ""}</span>
                <div className={s.divLine} />
              </div>
        ...
        <div className={s.certFooter}>
          ...
          <p className={s.footerUrl} style={{ paddingTop: 4 }}>lot.mes-fd.com</p>
        </div>
      </div>
    </div>
  )
