...
export default function WIPage({ onLogout, onBack }) {
  ...
  const handleMaterialSubmit = (sel) => { setSelections(sel); setLotNo(`WI${sel.worker}${date}`); setStep('count') }
  const handleCountSelect = (count) => { setPrintCount(count); setStep('confirm') }
  const handleConfirm = async () => {
    setPrinting(true)
    try { await printLot(lotNo, printCount, { selected_Process: 'WI', lot_chain: lotChain, ...selections }); setDone(true) }
  ...
  return (
    <>
      {step === 'qr' && <QRScanner processLabel="WI, 권선" onScan={async (val) => { try { const r = await scanLot('WI', val); setPrevLotNo(r.prev_lot_no); setLotChain(r.lot_chain); setStep('selector') } catch (e) { setError(e.message) } }} onLogout={onLogout} onBack={onBack} />}
      {step === 'selector' && <MaterialSelector steps={steps} autoValues={{ process: 'WI', date, seq: '00' }} onSubmit={handleMaterialSubmit} onLogout={onLogout} onBack={() => setStep('qr')} />}
