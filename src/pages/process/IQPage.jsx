...
export default function IQPage({ onLogout, onBack }) {
  ...
  const handleMaterialSubmit = (sel) => { setSelections(sel); setLotNo(`IQ${sel.worker}${date}`); setStep('count') }
  const handleCountSelect = (count) => { setPrintCount(count); setStep('confirm') }
  const handleConfirm = async () => {
    setPrinting(true)
    try { await printLot(lotNo, printCount, { selected_Process: 'IQ', lot_chain: lotChain, ...selections }); setDone(true) }
  ...
  return (
    <>
      {step === 'qr' && <QRScanner processLabel="IQ, 수입검사" onScan={async (val) => { try { const r = await scanLot('IQ', val); setPrevLotNo(r.prev_lot_no); setLotChain(r.lot_chain); setStep('selector') } catch (e) { setError(e.message) } }} onLogout={onLogout} onBack={onBack} />}
      {step === 'selector' && <MaterialSelector steps={steps} autoValues={{ process: 'IQ', date, seq: '00' }} onSubmit={handleMaterialSubmit} onLogout={onLogout} onBack={() => setStep('qr')} />}
