...
export default function SOPage({ onLogout, onBack }) {
  ...
  const handleConfirm = async () => {
    setPrinting(true)
    try { await printLot(lotNo, printCount, { selected_Process: 'SO', lot_chain: lotChain, ...selections }); setDone(true) }
  ...
  return (
    <>
      {step === 'qr' && <QRScanner processLabel="SO, 중성점" onScan={async (val) => { try { const r = await scanLot('SO', val); setPrevLotNo(r.prev_lot_no); setLotChain(r.lot_chain); setStep('selector') } catch (e) { setError(e.message) } }} onLogout={onLogout} onBack={onBack} />}
