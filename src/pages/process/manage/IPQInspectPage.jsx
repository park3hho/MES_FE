...
import RotorNcPage from '@/pages/process/manage/RotorNcPage'                 // 회전자 부적합 처리 (IPQ 진입, 2026-08-11)
...
export default function IPQInspectPage({ user, onLogout, onBack, entryLabel = 'IPQ — 공정검사', skipLineSelect = false }) {
  ...
  // 라인 선택 (2026-07-22) — OQ 와 동일: null=선택 전 / 'stator'(고정자) / 'rotor'(회전자).
  const [line, setLine] = useState(null)
  // 회전자 진입 후 작업 선택 (2026-08-05) — 측정과 폐기는 진입점 분리: null / 'inspect'(요크 검사) / 'discard'(요크 폐기).
  const [rotorMode, setRotorMode] = useState(null)
  ...
  const sequence = useMemo(() => {
    ...
    return base.filter((k) => {
      ...
      // skip_ec step — problem_process='BO' 일 때만 (= BO→HT 되돌릴 때 EC 다시? 묻기).
      if (k === 'skip_ec') {
  ...
  // 해당 step 에 진입할 때 비어있으면 불량 수량으로 미리 채움 → 일반 케이스(전수 귀책) 한번에 진행.
  useEffect(() => {
  ...
  if (!skipLineSelect && effLine === 'rotor') {
    ...
    if (!rotorMode) {
      return (
        <div className="page-flat" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '48px 16px', maxWidth: 420, margin: '0 auto' }}>
          ...
          <button className="btn-secondary btn-lg btn-full" onClick={() => setRotorMode('nc')}>부적합 처리</button>
    ...
    if (rotorMode === 'nc') {
      // 부적합 처리 = 스캔 → 단계 판별 → 불량유형 + 붙인 자석 입력 → 격리(NCR + 재고 nonconforming + 자석 차감).
      return <RotorNcPage onLogout={onLogout} onBack={() => setRotorMode(null)} />
    }
  ...
  function renderQuestion() {
    switch (key) {
      ...
      // IPQ 는 process_category='공정' (내부 가공) 이라 귀책 = 자체만.
      case 'defect_detail':
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <label style={{ ...cell, flex: 1 }}>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              // 빈칸도 0 으로 간주 (사용자가 지웠을 때 반대편 즉시 갱신).
        ...
        <label style={{ ...cell, flex: 1 }}>
          ...
          <input
            ...
            onChange={(e) => {
              // 불량 빈칸/0 → 양품 = 검사수량 (사용자가 지웠을 때 즉시 반대편 갱신)
              const v = e.target.value
              set('defect_qty', v)
              const n = v === '' ? 0 : parseFloat(v)
              if (!isNaN(insp) && !isNaN(n) && n >= 0 && n <= insp) {
