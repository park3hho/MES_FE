...
import {
  QC_JUDGMENT, RESPONSIBLE, HANDLE_METHOD,
...
export const REPAIR_LABEL_TO_CODE = Object.fromEntries(REPAIR_CATEGORIES.map((c) => [c.label, c.code]))
...
export function renderNgStep(stepKey, ctx) {
  switch (stepKey) {
          ...
          {(ctx.value || '').startsWith('기타') && (
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <BigInput type="text"
                ...
                autoFocus placeholder="기타 사유 (선택)"
    ...
    case 'responsible':
      ...
      return (
        <Question title="귀책 대상은?"
          footer={<GhostButton onClick={() => { ctx.setValue(''); ctx.goNext?.() }}>건너뛰기</GhostButton>}>
          <BigChoice
    ...
    case 'responsible_qty':
      return (
        <Question title="귀책 수량은?"
          footer={<>
            <PrimaryButton onClick={ctx.goNext}>다음</PrimaryButton>
            <GhostButton onClick={() => { ctx.setValue(''); ctx.goNext?.() }}>건너뛰기</GhostButton>
          </>}>
          <BigInput type="number" inputMode="numeric" min="0"
            max={ctx.maxQty || undefined} step="any"
            value={ctx.value} autoFocus placeholder="0"
            onChange={(e) => {
    ...
    case 'handle_method':
      return (
        <Question title="처리 방법은?"
          footer={<GhostButton onClick={() => { ctx.setValue(''); ctx.goNext?.() }}>건너뛰기</GhostButton>}>
          <BigChoice options={Object.values(HANDLE_METHOD)} value={ctx.value}
            onPick={(v) => ctx.pickAndNext?.(v) ?? (ctx.setValue(v), ctx.goNext?.())} />
        </Question>
      )

    case 'problem_process': {
      const opts = getProblemProcessOptions(ctx.detectedProcess).map((p) => `${p.key} (${p.name})`)
      const toKey = (label) => label.split(' ')[0]
      return (
        <Question title="어느 공정에서 문제가 발생했나요?" sub="재작업 LOT 가 그 공정 직전으로 되돌아갑니다">
          <BigChoice options={opts}
...
// ═════════════════════════════════════════════════════════════════════════
const NG_SEQ_BASE = ['defect_detail', 'responsible', 'responsible_qty', 'handle_method', 'problem_process', 'remark']
const NG_CHIP_META = {
...
export function NgFollowupWizard({
  ...
  const sequence = useMemo(() => NG_SEQ_BASE.filter((k) => {
  ...
  const goBack = () => { if (stepIndex > 0) setStepIndex(stepIndex - 1); else onCancel?.() }
  const jumpTo = (i) => setStepIndex(i)
  const pickAndNext = (k, v) => { set(k, v); setTimeout(goNext, 120) }

  // chips — 지나온 step 중 값 있는 것
  const lotChip = lotNo ? { key: 'lot', label: 'LOT', value: lotNo, onClick: () => onCancel?.() } : null
  const chips = [
    ...(lotChip ? [lotChip] : []),
  ...
  const respOpts = responsibleOptions || [RESPONSIBLE.SELF, RESPONSIBLE.SUPPLIER, RESPONSIBLE.OUTSOURCE]

  // 문제 공정 후보 (재작업 + 우리 LOT 일 때만 노출되니까 안전하게 계산)
  const problemProcessOpts = getProblemProcessOptions(detectedProcess).map((p) => `${p.key} (${p.name})`)
  ...
  function renderStep() {
    ...
    if (key === 'remark') {
      return (
        <Question title="비고가 있나요?" sub="특이사항 (선택). 입력 후 저장하세요"
          footer={<PrimaryButton onClick={handleSubmit} disabled={saving}>
            {saving ? '저장 중…' : submitLabel}
          </PrimaryButton>}>
          <BigInput type="text" value={ngForm.remark} autoFocus placeholder="특이사항"
