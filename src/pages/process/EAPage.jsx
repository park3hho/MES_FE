...
export default function EAPage({ onLogout, onBack }) {
  ...
  return (
    <>
      {step === 'qr' && <QRScanner processLabel="EA, 낱장가공" onScan={async (val) => { try { const r = await scanLot('EA', val); setPrevLotNo(r.prev_lot_no); setLotChain(r.lot_chain); setStep('selector') } catch (e) { setError(e.message) } }} onLogout={onLogout} onBack={onBack} />}
