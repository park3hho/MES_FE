...
//   모든 원자재(코일/EW/Plate/자석 등): RmItemWizard — 품목 선택 → 공급사 → 속성 → 입고일 → 발급
//     LOT = {Item.lot_material_code}-{vendor.code}-{attribute}-{YYMMDD}-{NN} (BE 채번)
//   종류는 원자재 Item 카테고리에서 getRmKinds 로 동적 생성 (2026-06-11 자석 전용 플로우 폐지 — 동적 통일).
...
export default function RMPage({ onLogout, onBack }) {
  ...
  if (!kind) {
    const all = kinds   // 원자재 Item 카테고리에서 동적 생성 (자석 포함, 2026-06-11)
    return (
