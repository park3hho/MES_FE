...
// CT = 코팅 (2026-09-12 옛 EC). EC 는 옛 코드 — 구간 판정·라벨만 호환 (순서는 CT)
const PROCESS_ORDER = ['RM', 'MP', 'EA', 'HT', 'BO', 'CT', 'WI', 'SO', 'OQ', 'FP', 'UB', 'MB', 'OB', 'RT']
// 제품 생애 구간 (RM→FP/RT) vs 출하 물류 구간 (UB→OB) 경계
const PRODUCT_PROCESSES = new Set(['RM', 'MP', 'EA', 'HT', 'BO', 'CT', 'EC', 'WI', 'SO', 'OQ', 'FP', 'RT'])
const SHIPPING_PROCESSES = new Set(['UB', 'MB', 'OB'])

const PROC_LABEL = {
  RM: '원자재', MP: '자재준비', EA: '낱장가공', HT: '열처리',
  BO: '본딩', CT: '코팅', EC: '코팅', WI: '권선', SO: '중성점',
...
export default function LifelineView({
  ...
  const renderNode = (ent) => {
    // 중간 세대(승계) 노드는 아예 표시 안 함 — suffix 길이가 원본도 최종도 아닌 것 (2026-07-31)
    if (ent.superseded) return null
